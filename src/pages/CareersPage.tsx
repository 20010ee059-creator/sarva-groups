    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-8 space-y-3">
          ...
          <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200">No Open Positions Currently</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {jobs.map((job) => (
            <div
              key={job.id}
              className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-xl space-y-4 flex flex-col justify-between"
            >
              <div className="space-y-3">
                ...
                <h3 className="text-lg font-bold text-slate-900 dark:text-white font-poppins">{job.title}</h3>
                <div className="flex items-center gap-2 text-xs text-slate-500">
                  <MapPin className="w-3.5 h-3.5 text-amber-500" />
                  <span>{job.location}</span>
                </div>
                <p className="text-xs text-slate-600 dark:text-slate-400">{job.desc}</p>
              ...
              <a
                ...
                className="w-full text-center bg-slate-900 dark:bg-slate-800 text-white font-bold text-xs py-2.5 rounded-xl hover:bg-slate-800 block"
              >
      ...
      {/* Application Form */}
      <div id="apply-form" className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 shadow-xl max-w-2xl mx-auto space-y-6">
        <h3 className="text-2xl font-bold text-slate-900 dark:text-white font-poppins text-center">
        ...
        {submitted ? (
          <div className="p-6 bg-emerald-50 dark:bg-emerald-950/40 rounded-2xl border border-emerald-200 dark:border-emerald-800 text-center space-y-2">
            ...
            <h4 className="text-lg font-bold text-emerald-800 dark:text-emerald-200">Application Submitted!</h4>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Applied Role</label>
                <select
            ...
            <div>
              <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Experience & Background Note</label>
              <textarea
                rows={3}
