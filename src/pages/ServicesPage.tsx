  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <motion.div
                ...
                className={`p-4 rounded-2xl border cursor-pointer transition-all flex items-center justify-between ${
                  isSelected
                    ...
                    : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 hover:border-amber-500/50'
                }`}
              >
                <div>
        ...
        <div className="lg:col-span-8 bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 shadow-2xl space-y-6">
          ...
          <div className="space-y-3">
            <h4 className="text-xs font-black uppercase tracking-wider text-slate-900 dark:text-white font-poppins">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {selectedService.benefits.map((b, idx) => (
                <motion.div
                  ...
                  className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-100 dark:border-slate-800 flex items-start gap-2.5 text-xs font-medium text-slate-800 dark:text-slate-200 cursor-pointer"
                >
          ...
          {selectedService.faqs && selectedService.faqs.length > 0 && (
            <div className="space-y-3 pt-4 border-t border-slate-200 dark:border-slate-800">
              <h4 className="text-xs font-black uppercase tracking-wider text-slate-900 dark:text-white font-poppins flex items-center gap-1.5">
              ...
              <div className="space-y-2">
                {selectedService.faqs.map((f, idx) => (
                  <div key={idx} className="p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl text-xs space-y-1">
                    ...
                    <p className="text-slate-600 dark:text-slate-300">A: {f.answer}</p>
                  </div>
                ))}
              </div>
            </div>
