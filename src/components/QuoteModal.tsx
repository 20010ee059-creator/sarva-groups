...
export const QuoteModal: React.FC<QuoteModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm overflow-y-auto animate-fadeIn">
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-2xl overflow-hidden my-8 relative">
        {/* Header Banner */}
        ...
        {submitted ? (
          <div className="p-8 text-center space-y-4">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-300 max-w-md mx-auto leading-relaxed">
              Thank you, <span className="font-bold text-slate-900 dark:text-white">{formData.name}</span>. Our senior solar engineer from Sarva Solar will contact you at <span className="font-bold text-amber-600">{formData.phone}</span> within 2 hours with detailed feasibility report.
            </p>

            <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 max-w-md mx-auto text-left space-y-2 text-xs">
              <div className="flex justify-between font-bold text-slate-700 dark:text-slate-200">
                <span>Recommended Capacity:</span>
                <span className="text-blue-600 dark:text-amber-400">{estimatedKw} kW System</span>
              </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            {errorMsg && (
              <div className="p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-xl text-red-600 dark:text-red-400 text-xs flex items-center gap-2">
            ...
            <div className="bg-gradient-to-br from-blue-50 to-emerald-50 dark:from-slate-800 dark:to-slate-800/80 p-4 rounded-2xl border border-blue-100 dark:border-slate-700 grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
              <div>
                <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400">Rec. Size</span>
                <p className="text-base font-extrabold text-blue-600 dark:text-amber-400">{estimatedKw} kW</p>
              </div>
              <div>
                <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400">Gross Price</span>
                <p className="text-base font-bold text-slate-700 dark:text-slate-200">₹{estimatedCost.toLocaleString('en-IN')}</p>
              </div>
              <div>
                <span className="text-[10px] uppercase font-bold text-emerald-600 dark:text-emerald-400">Govt Subsidy</span>
                <p className="text-base font-extrabold text-emerald-600 dark:text-emerald-400">₹{estimatedSubsidy.toLocaleString('en-IN')}</p>
              </div>
              <div>
                <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400">Net Cost</span>
                <p className="text-base font-black text-slate-900 dark:text-white">₹{netCost.toLocaleString('en-IN')}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2 text-sm focus:outline-none focus:border-amber-500 text-slate-900 dark:text-white"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-amber-500 text-slate-900 dark:text-white"
                >
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-amber-500 text-slate-900 dark:text-white"
              />
            </div>
